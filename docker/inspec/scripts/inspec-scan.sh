...
then
        ...
        do
                ...
                then
                        ...
                        if [ "$RUNVAL" = "hosts" ]
                        then
                                inspec exec $INSPEC_DIR -t vmware:// \
                                        --input $SCAN_SYSTEM=$(echo $SCAN_VAL | cut -d'"' -f 2) \
                                                syslogServer=$SYSLOG \
                                                ntpServer1=$NTP1 \
                                                ntpServer2=$NTP2 \
                                                exceptionUsers=${EXCEPTION_USER} \
                                       --show-progress \
                                       --reporter=cli \
                                       json:/scans/$SCAN_VAL_SHORT.json 2>> /logs/$RUNVAL-$LOG_TIME-error.log
                        elif [ "$RUNVAL" = "vms" ]
                        then
                                inspec exec $INSPEC_DIR -t vmware:// \
                                        --input $SCAN_SYSTEM=$(echo $SCAN_VAL | cut -d'"' -f 2) \
        ...
        then
                ...
                cd $INSPEC_DIR
                inspec exec $INSPEC_DIR/wrapper -t ssh://$SSH_USER@$VISERVER \
                        --password $SSH_PASS \
                        --input syslogServer=$(echo $SYSLOG | sed -E 's|[a-z]{3}://||g') \
                                ntpServer1=$NTP1 \
                                ntpServer2=$NTP2 \
                        --show-progress \
                        --reporter=cli \
